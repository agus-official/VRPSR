...
from unet_with_meta import UNet2DConditionModelWithMeta, ResnetBlock2DWithMeta
...
class S3DiffMetaEmbed(torch.nn.Module):
    def __init__(self, sd_path=None, pretrained_path=None, lora_rank_unet=32, lora_rank_vae=16, block_embedding_dim=64, random_unet: bool = False):
        super().__init__()
        ...
        self.random_unet = random_unet

        unet = UNet2DConditionModelWithMeta.from_pretrained(
            sd_path,
            subfolder="unet",
            scalar_dim=16, hidden_dim=128, num_codec=3
        )

        if self.random_unet:
            def _reinit(m: torch.nn.Module):
            ...
            unet.apply(_reinit)
            print("[S3DiffMetaEmbed] UNet weights are RANDOM (reinitialized in-place).")
        ...
        if (pretrained_path is not None) and (not self.random_unet):
        # if pretrained_path is not None:
            ...
            filtered = {k: v for k, v in sd["state_dict_unet"].items()
                        if k in state and v.shape == state[k].shape}

            missing, unexpected = unet.load_state_dict(filtered, strict=False)

            print(f"[LoRA] skipped {len(sd['state_dict_unet']) - len(filtered)} keys "
                f"due to shape mismatch; loaded {len(filtered)} keys OK.")
            ...
            if self.random_unet:
                print("[Init] random_unet=True: use random UNet (already reinit), add LoRA adapters.")
            else:
                print("[Init] No pretrained_path: base UNet from sd_path, add fresh LoRA adapters.")
    ...
    def forward(self, c_t, deg_score, prompt, meta_vec=None):
        ...
        model_pred = self.unet(
            encoded_control, self.timesteps,
            encoder_hidden_states=caption_enc,
            meta_vec=meta_vec  # Embed
        ).sample
        ...
        self.intermediate_outputs['c_t'] = c_t.detach().cpu()  
        self.intermediate_outputs['input_deg_score'] = deg_score.detach().cpu() 
        self.intermediate_outputs['prompt'] = prompt 
        self.intermediate_outputs['meta_vec'] = meta_vec
        
        self.intermediate_outputs['caption_enc'] = caption_enc.detach().cpu()
        self.intermediate_outputs['deg_proj'] = deg_proj.detach().cpu()
        self.intermediate_outputs['vae_embeds'] = vae_embeds.detach().cpu()
        self.intermediate_outputs['unet_embeds'] = unet_embeds.detach().cpu()
        self.intermediate_outputs['encoded_control'] = encoded_control.detach().cpu()
        self.intermediate_outputs['model_pred'] = model_pred.detach().cpu()
        self.intermediate_outputs['x_denoised'] = x_denoised.detach().cpu()

        self.intermediate_outputs['output_image'] = output_image.detach().cpu()


        return output_image, self.intermediate_outputs 
    ...
    def save_model(self, outf):
        ...
        # ---------- UNet Weights: Save LoRA + conv_in + meta-mlp ----------
        if self.random_unet:
            sd["state_dict_unet"] = self.unet.state_dict()
        else:
            unet_state = self.unet.state_dict()
            sd["state_dict_unet"] = {
                k: v for k, v in unet_state.items()
                if "lora_" in k or "conv_in" in k or "add_embedding" in k or ".mlp." in k
            }

        # ---------- VAE ----------
        sd["state_dict_vae"] = {
            k: v for k, v in self.vae.state_dict().items()
            if "lora_" in k or "skip_conv" in k
        }

        # Degradation MLPs
        sd["state_dict_vae_de_mlp"]   = self.vae_de_mlp.state_dict()
        sd["state_dict_unet_de_mlp"]  = self.unet_de_mlp.state_dict()

        # Block Embedding MLPs
        sd["state_dict_vae_block_mlp"]  = self.vae_block_mlp.state_dict()
        sd["state_dict_unet_block_mlp"] = self.unet_block_mlp.state_dict()

        # Fuse MLPs
        sd["state_dict_vae_fuse_mlp"]   = self.vae_fuse_mlp.state_dict()
        sd["state_dict_unet_fuse_mlp"]  = self.unet_fuse_mlp.state_dict()
        ...
        torch.save(sd, outf)
        print(f"[SAVE] Model saved to {outf}")
        print(f"  - UNet saved: {len(sd['state_dict_unet']):3d} keys (lora/conv_in/mlp/add_embedding)")
        print(f"  - VAE  saved: {len(sd['state_dict_vae']):3d} keys (lora/skip_conv)")