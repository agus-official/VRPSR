...
def random_generate_gaussian_noise_pt(img, sigma_range=(0, 10), gray_prob=0, seed=None):
    ...
    return generate_gaussian_noise_pt(img, sigma, gray_noise, seed=seed)
...
def random_add_gaussian_noise_pt(img, sigma_range=(0, 1.0), gray_prob=0, clip=True, rounds=False, seed=None):
    ...
    noise = random_generate_gaussian_noise_pt(img, sigma_range, gray_prob, seed=seed)
    out = img + noise
    if clip and rounds:
        out = torch.clamp((out * 255.0).round(), 0, 255) / 255.
    elif clip:
...
def random_add_poisson_noise_pt(img, scale_range=(0, 1.0), gray_prob=0, clip=True, rounds=False, seed=None):
    ...
    noise = random_generate_poisson_noise_pt(img, scale_range, gray_prob, seed=seed)
    out = img + noise
    if clip and rounds:
        out = torch.clamp((out * 255.0).round(), 0, 255) / 255.
    elif clip:
