...
from basicsr.archs.rrdbnet_arch import RRDBNetEmbeddingProj
...
def main(args):
    ...
    if args.model == "s3diff":
        ...
        # Real-ESRGAN
        net_sr = RRDBNetEmbeddingProj(num_in_ch=3, num_out_ch=3, scale=1, num_feat=64, num_block=23, num_grow_ch=32)
        if args.pretrained_sr_path is not None:
            ckpt_obj = torch.load(args.pretrained_sr_path, map_location="cpu")
        sr_state = ckpt_obj.state_dict()
        net_sr.load_state_dict(sr_state, strict=False)

    print(f"[init] Weights loaded from {args.pretrained_sr_path}")
    ...
    for step, batch_val in enumerate(dl_val):
        ...
        with torch.no_grad():
            ...
            if args.model == "s3diff":
                ...
                x_sr_pred = accelerator.unwrap_model(net_sr)(x_src * 0.5 + 0.5, temb=temb)

            # Step 2: Real H264: x_sr_pred --> x_codec_pred 
            best_qp, best_bpp, x_codec_pred = find_best_qp(
